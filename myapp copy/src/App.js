    ...
    <Router>
      <Container>
        ...
        >
          {serviceList.map((service) => (
            <Paper elevation={3} sx={{ width: { xs: 1, md: 320 } }} key={service.name}>
              <Box sx={{ m: 3 }}>
                ...
                <Typography sx={{ m: 2 }}>{service.description}</Typography>
                <button onClick={() => {navigate("/simulations");}}>Learn More</button>
              </Box>
            </Paper>
          ))}
        </Box>
          ...
          <Routes>
          {serviceList.map((service) => (
            <Route key={service.name} exact path={service.path} element={<Simulations name={service.name} />} />
          ))}
          </Routes>
